function [startDate, tspanY, ydata, ty_outlier, y_outlier] = aggBloodDataRaw(num, txt, patient, beforeTherapy)
    if ~exist('beforeTherapy','var')
        beforeTherapy=false;
    end
        
            [startDate,tspanY, ydata]=getPatientDataRaw(num, txt, patient, beforeTherapy);
            tspanY=tspanY';
            ydata=ydata';
            ty_outlier=[];
            y_outlier=[];
