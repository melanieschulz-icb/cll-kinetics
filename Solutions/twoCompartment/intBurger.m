function [x_out, y_out] = intBurger(par, tspanX, tspanY)
% Solution as defined in the Burger/Wodarz supplements (does not match the ODE)
%Outputs:
...
y_out = (m*A)/(B*d2) + ...
   ( m/(d2 - B)) * x0 *exp(-B*tspanY)  + ...
    (y0 - (m*A)/(B*d2) - ( m/(d2 - B)) * x0)*exp(-d2 * tspanY);
