...
%% Simulated Data
%par_slow = [0.002 0.027 0.0096 3034e9 153e9 0.4e9];
par_1 = [0.040,	0.0001,	0.0324,1.41e+12, 4.77e+11,2.54e+13]
par_2=[0.022,0.00018,0.02,1.78e+12,	5.85e+11,6.49e+12]
par_8=[0.031,5.13e-05,0.009,2.26e+12,4.83e+11,6.21e+13]
pars=[par_8];
...
for patient=[101]
    ...
    par_true=pars(patient-100,:);
    Xmulti=[Xmulti;[patient,0,0,0,par_true]];
    
    tSpanYBurger = [0,2,14,28,42,56,70,83,167,252,335,420];
    tSpanXavailableBurger=[0,80, 300];
    [x_true, y_true] = intLea(par_true, tSpanXavailableBurger, tSpanYBurger);
    
    Xdata = x_true;
    ydata = y_true;
    
    for noise=1:1
        for tss_measurements=tss_measurement
            for runn=runns
                if patient==12
                    'Skipping patient_nr 12 as therapy was paused in between';
                else
                    'Fitting Patient_nr '+ string(patient);
        %            close all; clearvars -except patient use_constraint patients Xmulti Xllh Errormulti Errorllh runn runns tss_measurements x normalize_lls tss_measurements
                    %% optimization using integral

                    %settings

                    lb = [1e-5 1e-5 1e-6 0.9 0.9 0]; 
                    ub = [5 20 1 1.1 1.1 1e12]; 
                    
                    if tss_measurements==1
                        n_starts = 500;
                    else
                        n_starts = 300;
                    end
                    ...
                    tspanXavailable=tSpanXavailableBurger;
                    tspanX=tspanXavailable(1:tss_measurements);


                    xdata=Xdata(1:tss_measurements);

                    xdata=Xnoise(noise,1:tss_measurements).*xdata;
                    ...
                    Xmulti=[Xmulti;[patient,noise,tss_measurements, runn, xmulti]];
        ...
        %             plot(tspanXavailable(1:tss_measurements), xdata, '*k'); hold on;
        ...
        %             plot(tSpanXavailableBurger, x_true, 'g'); hold on;
...
writetable(array2table(Xmulti, 'VariableNames',{'ACC','noise','tss_measurements','run','d2', 'd1','m','x0','y0','c'}),'Params_lss.xlsx');
