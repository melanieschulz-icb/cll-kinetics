function main_join_addPre_server(num, txt, chars, charDesc, patients, cll_volume, runns, tss_measurements, normalize_lls, use_complex_tss, mult_noise, opt_sd,folderbase,models,...
                       seperate_sd, refill, use_constraint,params, numPlat, txtPlat, hb_model,pre_therapy_models,combine_pre_post, bm_measurements,...
                       namesModelOne, ltPb, bmPb, ltBm,n_starts)
    for patient=patients
        Xmulti=[];
    %         mkdir(strcat(folderbase,'patient_',int2str(patient),'/'));
    %         folder=strcat("/",folderbase,'patient_',int2str(patient),'/');
    %         mkdir(strcat(folder.extractAfter(1),'/plots/'));
    ...
    %             'combine_pre_post','refill','n_starts');
        folderbase="/tes/";
        folder="/tes";
        save(strcat(pwd,folderbase,'tes.mat'),'folder');
        %% Set data
                ...
                for runn=runns
                    if patient==12
                       ...
                       close all; 
                        ...
                        [lb_p, ub_p,~, ~, par_init_p, ~] = getBounds(XdataAvailable(1),XdataAvailableRaw(end),ydata(tspanY==0),ZdataAvailable(1), model,patient,combine_pre_post);  
...
%                         savefig(f,strcat(pwd,plotFolder,plotTitle.String),'compact');
     ...
     %% Store Parameters and errors
        ...
        if opt_sd==false
            ...
            tabNames=["model","model_pre","ACC",namesModelOne,"m_PB_LT","m_PB_BM",...
            ...
            tab=array2table(Xmulti, 'VariableNames',tabNames.cellstr);
