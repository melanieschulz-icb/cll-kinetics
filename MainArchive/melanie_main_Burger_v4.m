...
%for patient=first_pat:last_pat
        if patient==12
            ...
            close all; clearvars -except num txt chars charDesc patient optimize_sd use_constraint patients cll_volume Xmulti Xllh Errormulti Errorllh runn runns tss_measurements x normalize_lls tss_measurements
            %% optimization using integral

            %settings

            lb = [1e-3 1e-3 1e-4 0.7 0.9 0]; lb_sd = [1e5 1e5];
            ub = [0.1 0.1 0.5 1.3 1.1 1e12]; ub_sd = [1e13 1e13];
            ...
            par_init = [0.01 0.5 0.000 0 0 1e10]; %start parameters
            ...
            %Burger ACC1
            [tspanY, ydata]=getPatientData(num, txt,chars, charDesc, patient);
            tspanY=tspanY';
            ydata=ydata';
            if patient == 3
            ...
            if optimize_sd == true
                startp_llh =  CustomStartPointSet(par_starts(:,1:8));
            else
               % startp_llh=CustomStartPointSet(par_starts(:,1:6));
               startp_llh=startpoints;
            end
...
featX = (xdata - x_out)./norm(xdata); %calculate differences
featY = (ydata - y_out)./norm(ydata); %calculate differences
