%% melanie_main_model 5
% can be used to fit the Burger data
% 3 Compartments
% uses optimistic lymphocyte counts witin the BM, assuming that amount of
% "good" cells remains constant
close all; clearvars;
...
patients=[5,6,8,15,17,19,20,22,23,28,30];
runns=1:3;
...
folder='/Figures/10_02_model5/imp/'
    ...
    for runn=runns
        if patient==12
           ...
           close all; 
            ...
            ub = [5 20 1 1.5 1.5 1.3 3 3 1e18 3]; 
...
%             end
            ...
            if normalize_lls==true
                if mult_noise==true
                    ...
                    lsqfun = @(par)diff_integral_norm_summed(10.^par,xdata,ydata,zdata,zdatanorm,tspanX, tspanY,tspanZ, weight);
                end
            else
                if mult_noise==true
                    if opt_sd==false
                        ...
                        lsqfun=@(par)diff_log_summed_sd_hier(10.^par,xdata,ydata,zdata,zdatanorm,tspanX, tspanY,tspanZ, weight);
                    end
                else
                    if opt_sd==false
                        lsqfun = @(par)diff_integral_summed(par,xdata,ydata,tspanX, tspanY, weight);
                    else
                        lsqfun = @(par)diff_integral_summed_sd_hier(10.^par,xdata,ydata,zdata,zdatanorm,tspanX, tspanY,tspanZ, weight);
                    end
                end 
            end
            ...
            elseif mult_noise==0 && opt_sd==1
                ...
                [sd]=compute_add_sd(xmulti,xdata,ydata,zdata,zdatanorm,tspanX, tspanY, tspanZ, weight);
                xmulti=[xmulti, sd];
                likelihood=likelihood_add_noise_model4(xmulti,xdata,ydata,zdata,zdatanorm,tspanX, tspanY,tspanZ, weight);
...
exportgraphics(f,[pwd,strcat(folder,plotTitle.String,'.png')],'Resolution',300)
            ...
            normal_cells=((1-zdata(1))/zdata(1))*xmulti(9);
            percentage=100*z_opt./(z_opt+normal_cells);

            plot(tspanz, percentage,'--m',xlabel('days'),ylabel('CLL cell percentage'));hold on;
            plot(tspanZAvailable, 100*zdata, '*k'); hold on;
            %plot(ty_outlier, y_outlier, 'og'); 
            legend('measurements','solution lsq')
...
function [sensitivity] = diff_integral_summed_sd_hier(par,xdata,ydata,zdata,zdatanorm,tspanX, tspanY,tspanZ, weight)
%Calculates the differences between the data and simulation output
%can be used as input for lsqnonlin/least squares optimizer
%outputs:                          
%   sensitivity = output feature, differences of both outputs
...
BM_cellCount=((1-zdata(1))/zdata(1))*z_out(1)./(1-zdata);
z_outPercentage=z_out./BM_cellCount;
z_outDecline=z_outPercentage/z_outPercentage(1);
...
function [sd] = compute_add_sd(par, xdata, ydata,zdata, zdatanorm, tspanX, tspanY,tspanZ,weight)
    [x_out, y_out, z_out] = intLea(par(1:10), tspanX, tspanY, tspanZ);
...
function [likelihood]=likelihood_add_noise_model4(par,xdata,ydata,zdata,zdatanorm,tspanX, tspanY,tspanZ, weight)
    [x_out, y_out, z_out] = intLea(par(1:10), tspanX, tspanY, tspanZ);
    featX = (xdata - x_out)/norm(xdata);
