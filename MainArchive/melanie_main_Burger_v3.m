...
%for patient=first_pat:last_pat
    if patient==12
        ...
        runn=runn+1;
        close all; clearvars -except num txt patient optimize_sd use_constraint patients cll_volume Xmulti Xllh Errormulti Errorllh runn tss_measurements
        %% optimization using integral

        %settings

        lb = [1e-3 1e-3 1e-6 0.9 0.9 0]; lb_sd = [1e5 1e5];
        ub = [0.5 20 1 1.1 1.1 1e12]; ub_sd = [1e13 1e13];


        spacing = 1; % 1 = linear; 2 = log10; 3 = log
        ...
        par_init = [0.01 0.02 0.000 0 0 1e10]; %start parameters
       ...
       % errormulti
        ...
        plot(tspan, x_opt, '--m',xlabel('days'),ylabel('absolute lymphocyte count')); 
