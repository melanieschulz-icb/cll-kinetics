    ...
    for runn=runns
        if patient==12
           ...
           close all; 
            ...
            lb = [1e-10 1e-15 1e-15 0.5 0.5 1e2 1e-15 1e-15 1e3 1e3]; 
            ub = [5 20 1 1.5 1.5 1.3 1 1 1e20 1e20]; 
...
%             end
            ...
            par_init = [0.1 0.1 0.1 1e10 1e10 1e10 0.1 0.1 1e10 1e10]; %start parameters
            sdXY=[20 20];
...
%             end
            ...
            elseif mult_noise==0 && opt_sd==1
                ...
                [sd_x, sd_y, sd_z]=compute_add_sd(xmulti,xdata,ydata,zdatanorm,tspanX, tspanY, tspanZ, weight);
                xmulti=[xmulti, sd_x];
                [likelihood,sd]=likelihood_add_noise_BM(xmulti,xdata,ydata,zdatanorm,tspanX, tspanY,tspanZ, weight);
            ...
            tspanx=0:1:500;
            tspanz=0:1:700;
            [x_opt, y_opt, z_opt] = intLea(xmulti(1:10), tspanx, tspan,tspanz);
...
if opt_sd==false
    ...
    writetable(array2table(Xmulti, 'VariableNames',{'ACC','d2', 'd1','m','x0','y0','c','d3','m2','z0','c2','sdx','sdy','sdz', 'bic', 'aicr'}),[pwd,strcat(folder,'Params.xlsx')]);
end
...
featZ = (zdatanorm - z_outDecline)./(norm(zdatanorm)*par(13));


sensitivity=length(xdata)*log(2*pi*par(11)^2)+length(ydata)*log(2*pi*par(12)^2)+length(zdatanorm)*log(2*pi*par(13)^2)+norm([featX, featY, featZ])^2;
...
function [sd_x, sd_y, sd_z] = compute_add_sd(par, xdata, ydata, zdatanorm, tspanX, tspanY,tspanZ,weight)
    [x_out, y_out, z_out] = intLea(par(1:10), tspanX, tspanY, tspanZ);
    ...
    sd_x=sqrt((1/length([xdata, ydata, zdatanorm]))*(norm([featX, featY, featZ])^2));
    sd_y=sd_x;
    sd_z=sd_x;
end
...
function [likelihood, sd]=likelihood_add_noise_BM(par,xdata,ydata,zdatanorm,tspanX, tspanY,tspanZ, weight)
    [x_out, y_out, z_out] = intLea(par(1:10), tspanX, tspanY, tspanZ);
    featX = (xdata - x_out)/norm(xdata);
