function main_joint(num, txt, chars, charDesc, patients, cll_volume, runns, tss_measurements, normalize_lls, use_complex_tss, mult_noise, opt_sd,folderbase,models,...
...
%         models=unique(models);
            ...
            for runn=runns
                if patient==12
                   ...
                   close all; 
                    ...
                    [lb_p, ub_p,lb_sd, ub_sd, par_init_p, sdXYZ] = getBounds(xdata(1), ydata(1),ZdataAvailable(1), model,patient);  
    ...
    %                             lsqfun = @(par)diff_integral_summed_sd(10.^par,xdata,ydata,zdata,tspanX, tspanY,tspanZ, model, refill,seperate_sd,HB',tspanYPlats', hb_model);
         ...
         %          aineq(2,length(names)+2)=-1;
                   ...
                   'lb',lb,'ub',ub,'Aineq',aineq,'bineq',[0;0;0]);
...
%                     savefig(f,strcat(pwd,plotFolder,plotTitle.String),'compact');
     ...
     %% Store Parameters and errors
        ...
        if opt_sd==false
            ...
            tab=array2table(Xmulti, 'VariableNames',["model","ACC",namesModelOne,"m_PB_LT","m_PB_BM",...
