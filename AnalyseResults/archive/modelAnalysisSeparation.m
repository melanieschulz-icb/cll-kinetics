...
for patient = patients
    ...
    [~, d_LT]=find(~cellfun(@isempty, regexp(names, 'd_LT')));
    d_LT=d_LT+d_PB-1;
    [~, m_LT_PB]=find(~cellfun(@isempty, regexp(names, 'm_LT_PB')));
    m_LT_PB=m_LT_PB+d_PB-1;
    [~, c_LT]=find(~cellfun(@isempty, regexp(names, 'c_LT')));
    c_LT=c_LT+d_PB-1;
    [~, d_BM]=find(~cellfun(@isempty, regexp(names, 'd_BM')));
    d_BM=d_BM+d_PB-1;
    [~, m_BM_PB]=find(~cellfun(@isempty, regexp(names, 'm_BM_PB')));
    m_BM_PB=m_BM_PB+d_PB-1;
    [~, c_PB]=find(~cellfun(@isempty, regexp(names, 'c_PB')));
    c_PB=c_PB+d_PB-1;
    [~, c_BM]=find(~cellfun(@isempty, regexp(names, 'c_BM')));
    c_BM=c_BM+d_PB-1;
    [~, m_LT_BM]=find(~cellfun(@isempty, regexp(names, 'm_LT_BM')));
    m_LT_BM=m_LT_BM+d_PB-1;
...
%     BestModelFirstModel=[BestModelFirstModel;patient, bestBicFirstModels];
    ...
    for modelId=goodModels
        ...
        modelParams(counter,19)=unique(min(allNums(allNums(:,model)==modelId,measure)));
        modelParams(counter,20)=unique(max(allNums(allNums(:,model)==modelId,likelihood)));
...
%     RejectedParams=[RejectedParams; patient,rejectedParams];
 ...
 writetable(array2table(GoodModels, 'VariableNames',["patient","model","d_PB","d_LT","m_LT_PB","c_LT","d_BM","m_BM_PB", "c_BM","m_LT_BM","c_PB","m_PB_LT","m_PB_BM","m_BM_LT","BIC","lik"]),strcat(pwd,basefolder,'ModAnalysis','.xlsx'));
