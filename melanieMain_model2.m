...
models = [51,52,55,58,59,61,63,65]; %:66;
...
for patient=patients
    ...
    for model=models
        for runn=runns
            if patient==12
               ...
               clearvars -except num txt chars charDesc patient use_constraint patients cll_volume Xmulti Xllh Errormulti Errorllh runn runns tss_measurements x normalize_lls tss_measurements use_complex_tss weight mult_noise opt_sd folder folderbase model models seperate_sd refill params
               close all; 
                ...
                storeParams=zeros(1,14);
                storeParams(getRelevantParams(model))=xmulti;
                storeParams(12:14)=[sd_x, sd_y, sd_z];
                Xmulti=[Xmulti;[model,patient,storeParams,bic, aicr]];
                ...
                plot(tspanXAvailable(1:tss_measurements), XdataAvailable(1:tss_measurements), '*k'); hold on;
                ...
                yyaxis right;
                if refill
                    bm_cells=(z_opt(1)/zdata(1)).*ones(1,length(tspanz));
                else
                    normal_cells=((1-zdata(1))/zdata(1))*z_opt(1);
                    bm_cells=z_opt+normal_cells;
                end
                plot(tspanz, 100*z_opt./bm_cells,'--r',xlabel('days'),ylabel('CLL cell percentage'));
                plot(tspanZ, 100*zdata, '*k'); hold on;
                %outliner not taken into account
 ...
 %% Store Parameters and errors
    ...
    if opt_sd==false
        ...
        writetable(array2table(Xmulti, 'VariableNames',["model","ACC",varNames,"sdx","sdy","sdz", "bic", "aicr"]),[pwd,strcat(folder,'Params.xlsx')]);
    end
end
